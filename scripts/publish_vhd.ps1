...
Write-Debug "az storage blob directory list -c system -d `"Microsoft.Compute/Images/images`" --account-name $($storageAccount.name) --account-key `"${storageAccountKey}`" --query `"[?ends_with(@.name, 'vhd')].name`" -o tsv"
