...
resource azurerm_virtual_machine_extension pipeline_agent {
  ...
  protected_settings           = jsonencode({
    "commandToExecute"         = "powershell.exe -ExecutionPolicy Unrestricted -Command \"Copy-Item C:/AzureData/CustomData.bin ./prepare_agent.ps1 -Force;./prepare_agent.ps1 -AgentName ${var.pipeline_agent_name} -AgentPool ${var.pipeline_agent_pool} -AgentVersionId ${var.pipeline_agent_version_id} -Organization ${var.azdo_org} -PAT ${var.azdo_pat} *> C:/WindowsAzure/Logs/prepare_agent.log\""
  })
